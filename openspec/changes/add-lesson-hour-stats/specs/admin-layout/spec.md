...
### Requirement: 侧边栏菜单
系统 SHALL 在主体布局左侧渲染一个垂直导航菜单，支持收起/展开切换，展示系统名称和菜单项列表。菜单项 SHALL 包含"首页"、"绩效统计"、"消课表"和"课时统计"四个入口。
...
#### Scenario: 课时统计菜单项可见
- **WHEN** 用户查看侧边栏菜单
- **THEN** 菜单中显示"课时统计"条目，图标为 PieChartOutlined（或同类统计图标），点击导航至 `/admin/lesson-hour-stats`
